...
if __name__ == '__main__':
    ...
    for dataset in ['breastmnist', 'bloodmnist', 'chestmnist', 'dermamnist', 'octmnist', 'organamnist', 'organcmnist',
        ...
        for img_size in [28,64,128,224]:
            df = pd.DataFrame(columns=["dataset", "img_size", "Acc_Test", "Bal_Acc", "AUC", "CO"])
            # Extract the dataset and its metadata
            info = INFO[dataset]
            ...
            for corruption in corruptions:
                ...
                if (img_size == 28 and (
                    ...
                    test_loader = DataLoader(corrupted_test_test, batch_size=config['batch_size_eval'], shuffle=False,
                                             num_workers=4
                                             , worker_init_fn=seed_worker, generator=g)
                    y_pred = evaluate(config,dataset=dataset,test_loader=test_loader, model=model)
                    ...
                    for severity in range(5):
                        ...
                        acc = get_ACC(y_true, y_pred.cpu().numpy()[index_range], config['task'])
                        auc = get_AUC(y_true, y_pred.cpu().numpy()[index_range], config['task'])
                        bal_acc = get_Balanced_ACC(y_true, y_pred.cpu().numpy()[index_range], config['task'])
                        co = get_Cohen(y_true, y_pred.cpu().numpy()[index_range], config['task'])
