...
def multi_head_multi_domain_training_gradient_accumulation(config: dict, loader_dict):
    """
    ...
    for epoch in range(config['epochs']):
        ...
        for i in range(sum_loader_length_train):
            ...
            for x in range(gradient_accumulation_steps):
                ...
                labelsx = labels[config["MAX_GPU_BATCH_SIZE"] * x:config["MAX_GPU_BATCH_SIZE"] + config[
                    "MAX_GPU_BATCH_SIZE"] * x]
                #If the last batch is reached the Dataloader might be less then 64 and you have to break out of the loop,
                # if there is nothing in imagex and lablesx
                if imagesx.shape[0]==0:
                    break
                # Map the data to the available device
                imagesx = imagesx.to(device)
            ...
            # Compute the loss and perform backpropagation
                ...
                if task_dict[random_dataset] == 'multi-label, binary-class':
                    ...
                    outputs_detached = prediction(outputs_detached).to(device)
                    labelsx = labelsx.float().resize_(len(labelsx), 1)
                y_true_train_dict[random_dataset] = torch.cat((y_true_train_dict[random_dataset], deepcopy(labelsx)), 0)
        ...
        for dataset_name in dataset_names:
            ...
            Bal_Acc = get_Balanced_ACC(y_true_val_dict[dataset_name].cpu().numpy(), y_pred_val_dict[dataset_name].cpu().numpy(),
                          task_dict[dataset_name])
            ...
            Prec = get_Precision(y_true_val_dict[dataset_name].cpu().numpy(), y_pred_val_dict[dataset_name].cpu().numpy(),
                          task_dict[dataset_name])
